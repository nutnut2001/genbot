...
async def on_ready():
    await venady.change_presence(
        activity=disnake.Activity(
            ...
            name=f'*by gg/nuttools | {prefix}help'),
        ...
        if stock == None:
            ...
            if stock not in os.listdir(f"Accounts//"): 
                ...
                if len(lines) == 0: 
                    ...
                    try:
                        em = disnake.Embed(title = " ", description = f"*This is your generated account* \n `{str(account)}` \n *The Order is Email:Password:Secquestionanswer*", color = 0xFFFFFF, timestamp= ctx.message.created_at)
...
async def help(ctx):
    ...
    item = disnake.ui.Button(style=disnake.ButtonStyle.green, label="Support", url="https://discord.gg/nuttools")
    view.add_item(item=item)

    item2 = disnake.ui.Button(style=disnake.ButtonStyle.blurple, label="Invite Me", url="https://discord.com/api/oauth2/authorize?client_id=1076904143500877885&permissions=8&scope=bot")
...
async def invite(ctx):
    ...
    await ctx.message.reply("Server Invite: https://discord.gg/nuttools \n Bot invite: https://bit.ly/3WlC3Er \n Sourcecode: https://github.com/")
...
async def test(ctx):
    ...
    await ctx.message.reply("Private Command :P |Bot by discord.gg/nuttools")
